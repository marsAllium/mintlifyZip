...
Enriched bitcoin inputs contain input data enirched with the address that it belonged to (from previous unspent output) and the value of the input utxo. These are represented in `address0` and `value` fields respectively.
...
This table is indexed on address0, spent\_utxo\_id, block\_number, block\_timestamp, transaction\_hash. Query for a specific row with any of these columns to see faster query results.
...
| address0                 | address the input came from                                           | bc1...                                                               |
| value                    | value of the input                                                    | 1000                                                                 |
